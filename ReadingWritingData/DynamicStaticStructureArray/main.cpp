...
void wrtBin(fstream &out,Array a){
    //Write to the bin the address of the a.size variable and cast it as a pointer
    // to char
    out.write(reinterpret_cast<char *>(&a.size),sizeof(int));
    //a.data is already a pointer so the pointer itself is the address
    // cast is as a pointer to a char and store a certain amount of data (a.size*sizeof(int))
