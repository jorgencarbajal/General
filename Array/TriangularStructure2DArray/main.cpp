...
TriMatx *fillMat(int rows){
    ...
    triMt->size = rows;
    //creating the columns array and the double pointer
    // array
    ...
    //filling the columns with random numbers
    for (int i = 0; i < rows; i++){
        triMt->col[i] = mrkRand()%9+1;
    }

    //Filling each row of the double pointer array with the corresponding
    // necessary columns
    for (int j = 0; j < rows; j++){
        //Row one will have this many elements
        //Row two will have this many elements
        //Row three will have this many elements
        triMt->data[j] = new int[triMt->col[j]];
    }

    //Fill the data structure with shit
    for (int i = 0; i < triMt->size; i++){
    ...
    for(int i = 0; i < (triMt->size - 1); i++){
        for(int j = (i + 1); j < triMt->size; j++){
            if (triMt->col[triMt->indx[i]] > triMt->col[triMt->indx[j]]){
                int temp = triMt->indx[i];
                triMt->indx[i] = triMt->indx[j];
                triMt->indx[j] = temp;
            }
        }
    }
